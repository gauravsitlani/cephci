...
def sendEmailNew(def testResults){
    ...
    body += "<h2><u>Test Artifacts</h2></u><table><tr><td> RHEL7_COMPOSE_URL </td><td>${content."${build_action}".composes["rhel-7"]}</td></tr><td> RHEL8_COMPOSE_URL </td><td>${content."${build_action}".composes["rhel-8"]}</td><td>PRODUCT</td><td>${ciMsg.artifact.name}</td></tr>"
